...
class RoverFastTest extends TestCase
{
    public function testInvalidDirection(): void
    {
        ...
        new RoverFast(new Point(0, 0), 'invalid');
    }
    ...
    ): void
    {
        ...
        $rover = new RoverFast(new Point($startX, $startY), $startDirection);
        $rover->$method();
    ...
    public function provideTestMovementData(): array
    {
        return [
            ...
            ['forward', 10, -3, 'S', 10, -5, 'S'],
            ['forward', 10, -3, 'E', 13, -3, 'E'],
            ['forward', 10, -3, 'W', 8, -3, 'W'],

            // backward from origin
            ['backward', 0, 0, 'N', 0, -2, 'N'],
            ['backward', 0, 0, 'S', 0, 3, 'S'],
            ['backward', 0, 0, 'E', -2, 0, 'E'],
            ['backward', 0, 0, 'W', 3, 0, 'W'],

            // backward from (10, -3)
            ['backward', 10, -3, 'N', 10, -5, 'N'],
            ['backward', 10, -3, 'S', 10, 0, 'S'],
            ['backward', 10, -3, 'E', 8, -3, 'E'],
            ['backward', 10, -3, 'W', 13, -3, 'W'],
